...
| **エンドユーザー向け** | `/p/[slug]`(アイデア/登録), `/r/[token]`(順位), 埋め込みウィジェット | **neo 構造＋暖色**（開発者と同じネオ構造で配色だけ暖色に。ワクワク系。配色5案を `/preview/enduser-palettes` で探索済み・**未確定**） | `docs/DESIGN-ENDUSER.md`（定義予定。ブランチ `design/enduser-palettes`） |
...
- **メール（`api/`）は token 対象外**（Webフォント/CSS変数/影が効かない）。ただし**開発者向けメール**（ログインリンク・招待通知・コード送付・却下）は neo ブランド（藍×オレンジのワードマーク＋極太ボーダー）を inline hex で寄せる（`api/src/lib/email.ts` の `neoMail*` ヘルパ）。**エンドユーザー向けメール**（ウェイトリスト確認/完了/解除）は別系統＝従来のまま据え置き。
...
- 旧 M3ブルー系トークン（下記 §2）は **エンドユーザー系（`/p`・`/r`）と privacy** が使用。エンドユーザーは専用デザイン（neo 構造＋暖色・下表）へ移行予定。
