...
| **エンドユーザー向け** | `/p/[slug]`(アイデア/登録), `/r/[token]`(順位), 埋め込みウィジェット | **別デザイン（未定。Stitch で策定予定）** | `docs/DESIGN-ENDUSER.md`（定義予定） |
...
- **色トークンは `globals.css` の `@theme`/`.dark`（または系別スコープクラス）に置く**＝Tailwind が確実に出力する。
- **authored CSS クラス（タイポ・共有体裁）は `globals.css` に置かない**。Turbopack dev が globals.css の authored クラスを serve しないキャッシュ問題があったため、**共有 CSS は `src/app/brand.ts` の定数に集約**し、各ページの `<style>` がそれを読む（例: `brand-wordmark` / `brand-wordmark-strong`）。
- **1つの値を2か所以上に手書きしない**（ドリフトの温床。実際に LP/管理でロゴの太さ・ヘッダ高さがズレた）。

### 現状（移行中・2026-06-03 時点の負債）
- 開発者 neo の色が今 `.slp`(LP) / `.dash`(ダッシュ) / `.lhx`(chrome) の各スコープに**ハードコピー**されている＝原則3違反の負債。→ **`globals.css` の単一ソースへ集約する作業が次タスク**。
- 旧 M3ブルー系トークン（下記 §2）は login / プロジェクト詳細 / entries / privacy が今も使用。neo へ順次移行する。
