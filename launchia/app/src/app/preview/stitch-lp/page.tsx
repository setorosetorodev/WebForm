...
/**
 * Stitch「Landing Page - Reproduction & Alignment」(デザインシステム "Vibrant Dev-Pulse")の
 * 意匠を忠実に再現しつつ、コンテンツを Launchia の【実機能だけ】に差し替えた LP プレビュー。
 * 本番 `/`(src/app/page.tsx) とは別物・見比べ用。
 *
 * Stitch が混ぜてきた架空機能は実態に直してある:
 *  - リファラル/紹介で順位UP → 落とす（実在しない。将来 Phase2 候補）
 *  - GitHub 連携認証 → メール Magic Link + ダブルオプトインに
 *  - 先行アクセス/限定バッジ/段階ロールアウト → 「自分の順位を確認」へ
 *  - "無料で登録"/"ドキュメントを見る"/"5分で完了"/"1,234人" → 招待制・実在リンクのみ・断定回避
 *  - フッター Documentation/Pricing/Terms → プライバシーポリシーのみ
 *
 * - 配色は既存 semantic トークンを汚さないよう preview 内に閉じた CSS 変数で定義（Vibrant Dev-Pulse）。
 * - ダーク対応: 右下トグルで切替（`.dark` クラス）。Stitch の「紫ダーク」は採らず、青ブランドを保ったまま
 *   素直に暗転（方針 B）。ネオブルータリズムの枠＋ハード影はダークでは off-white インクに反転し、
 *   暗背景でも形が見えるようにしている。
 * - フォント(Lexend/Geist/JetBrains Mono)・Material Symbols は Google CDN を読み込み。
 */
export const metadata: Metadata = {
  title: 'Launchia LP 案（Stitch / Vibrant Dev-Pulse プレビュー）',
}
...
.slp {
  ...
  --slp-fg-soft: #464555;
  --slp-fg-faint: #767587;
  --slp-ink: #1b1b1f;                       /* 枠線・ハード影・見出しインク・区切り線 */
...
.dark .slp {
  /* ダーク（青ブランド維持・方針 B）。インクは off-white に反転して枠/影を可視化 */
  --slp-bg: #131316;
  --slp-surface: #1f1f29;
  ...
  --slp-fg-soft: #bdb9cc;
  --slp-fg-faint: #908ca0;
  --slp-ink: #3a3a48;                       /* 枠/影は純白でなく濃グレー（白すぎ回避＝ハイコントラスト感を抑える） */
...
export default function StitchVibrantLpPreview() {
  return (
